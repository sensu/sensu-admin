...
* Say they have no aggregates if array is empty
* Silence events en mass
* Make sure that downtimes and silence events have pre defined descriptions
* Silence all
* fix no space output from a check causing problems on events list
...
* Lazy load events, clients, etc
* Environments are an option
* Remove primary_role completely
